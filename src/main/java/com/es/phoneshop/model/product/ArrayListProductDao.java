...
public class ArrayListProductDao implements ProductDao {
    ...
    public synchronized List<Product> findProducts() {
        return products.stream().filter(x -> x.getPrice() != null && x.getStock() > 0)
                .collect(Collectors.toList());
    }

    @Override
    public synchronized void save(Product product) {
        if (products.stream().anyMatch(x -> x.getId().equals(product.getId()))){
            throw new IllegalArgumentException("Product with  id " + product.getId() + " exists");
        }else {
    ...
    public synchronized void delete(Long id) {
        if (!products.removeIf(p -> p.getId().equals(id))){
            throw new IllegalArgumentException("Product with  id " + id.toString() + " not exists");
        }
